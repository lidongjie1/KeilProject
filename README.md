...
4. PID调参心得
   1. 先调直立环，再调速度环
   2. 直立环调参
      1. 先调节Kp的极性和大小，再调节Kd的极性和大小
      2. 调节Kp(此时Kd为0，速度环的Kp和Ki都为0)
         1. 极性调节：可设置为1或者-1
            1. 极性正确：小车往哪边倒，车轮就往哪边加速转动，保证小车处于直立状态。
            2. 极性错误：小车往哪边倒，车轮就往反方向转动，会使得小车加速倒下。
         2. 调节Kp大小
            1. 一直增大kp直至小车出现大幅度低频率的震荡，此时kp已经足够大可以引入Kd
      3. 调节Kd
         1. 极性调节：可设置为1或者-1
            1. 极性正确：拿起小车绕电机轴旋转，车轮同向转动，有跟随。
            2. 极性错误：拿起小车绕电机轴旋转，车轮反向转动，无跟随。
         2. 调节Kd大小
            1. 一直增大kd，观察到小车出现不明显的小幅度高频率震荡时，可继续增大至出现明显的小幅度高频率的震荡
      4. 此时直立环的kp、kd参数都是最大值，需要乘以0.6最为最终参数。
   3. 速度环调参
      1. 先调节Kp的极性和大小，再调节Ki的极性和大小
      2. 调节Kp(注释掉直立环)
         1. 极性调节：可设置为1或者-1
            1. 极性正确：手动转动其中一个车轮，两个车轮会同向加速，直至电机最大速度
            2. 极性错误：手动转动其中一个车轮，另一车轮会以同样速度反向旋转
         2. 大小调节（取消注释掉直立环）
            1. 增大Kp&Ki，直至小车保持平衡且速度接近0。
      3. 调节Ki
         1. kp和ki呈线性关系：Ki=(1/200)*Kp,由此表达式可知kp，ki极性相同，故只需调节kp即可
